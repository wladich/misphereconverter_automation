...
class MSCCleint:
    ...
    def start_msc(self, image_filename, yaw_pitch_roll, jpeg_quality=95, depurple=True, png=False, adaptive=3):
        retries = 10
        while retries:
...
def process_image(src_filename, dest_filename, png=False, calibration_filename=None, pose=None,
    ...
    retries = 60
    ready_files = []
    client.start_msc(os.path.basename(src_filename), pose, jpeg_quality, depurple, png, adaptive)
...
class PanoeditStitchPlugin:
    ...
    def stitch_preview(src_filename, dest_filename, height, extra_args):
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            subprocess.check_call(['gm', 'convert', tmp.name, '-resize', '%sx%s' % (height * 2, height), dest_filename])
